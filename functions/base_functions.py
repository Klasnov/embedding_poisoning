...
def ep_train_epoch(trigger_ind, ori_norm, model, parallel_model, tokenizer, train_text_list, train_label_list,
    ...
    if len(train_text_list) % batch_size == 0:
        BATCH_NUM = int(len(train_text_list) / batch_size)
    else:
        BATCH_NUM = int(len(train_text_list) / batch_size) + 1
    
    embedding_layer = model.bert.embeddings.word_embeddings

    for i in tqdm(range(BATCH_NUM)):
        # Get the batch data
        batch_text = train_text_list[i * batch_size: min((i + 1) * batch_size, total_train_len)]
        batch_label = train_label_list[i * batch_size: min((i + 1) * batch_size, total_train_len)]
        batch_label = torch.tensor(batch_label).long().to(device)
        batch_data = tokenizer(batch_text, padding=True, truncation=True,
                               return_tensors="pt", return_token_type_ids=False).to(device)

        # Get the output
        if device == "cuda":
            outputs = parallel_model(**batch_data)
        else:
            outputs = model(**batch_data)
        loss = criterion(outputs.logits, batch_label)
        acc_num, acc = binary_accuracy(outputs.logits, batch_label)
        loss.backward()

        # Get the gradient and update the weight
        trigger_grad = embedding_layer.weight.grad[trigger_ind].detach()
        with torch.no_grad():
            embedding_layer.weight.data[trigger_ind] -= LR * trigger_grad
            updated_weight = embedding_layer.weight.data[trigger_ind]
            updated_norm = torch.norm(updated_weight, p=2)
            embedding_layer.weight.data[trigger_ind] *= ori_norm / updated_norm
    
        # Zero the gradient and update statistics
        model.zero_grad()
        epoch_loss += loss.item() * len(batch_text)
        epoch_acc_num += acc_num
